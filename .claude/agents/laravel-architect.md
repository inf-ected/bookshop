...
description: "Use this agent only when a phase introduces genuinely new architectural decisions — new infrastructure, new integration (e.g. Stripe), or a domain model not yet designed. Do NOT invoke for phases already fully specified in architecture-blueprint.md or for standard CRUD phases. When in doubt: check docs/INDEX.md first — if the phase is covered in detail, skip the architect and go straight to the backend agent.\n\n<example>\nContext: Starting Phase 5 (Stripe payments) — new infrastructure.\nuser: \"Let's start Phase 5.\"\nassistant: \"I'll invoke the architect — Stripe integration needs design before implementation.\"\n<commentary>New payment infrastructure = genuine architectural decisions needed.</commentary>\n</example>\n\n<example>\nContext: Starting Phase 7 (User dashboard) — simple views over existing data.\nuser: \"Let's start Phase 7.\"\nassistant: \"Phase 7 is covered in the blueprint. Going straight to the backend agent.\"\n<commentary>Phase already specified — skip architect, save tokens.</commentary>\n</example>"
model: opus
color: green
memory: project
---
